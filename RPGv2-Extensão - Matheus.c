...
void reinicializarJogo(int *enemyIndex, int *nMobs, const char *mobs[], bool *bossFinal, int *playerLvl, int *enemyLvl, int *contraataque, int *burnMago, bool *levelUp) {
    *enemyIndex = rand() % *nMobs;

    // Mensagem indicando o início de uma nova partida
...
void checkWin(int HPAtual, int enemyHP, bool *jogando, bool *levelUp, int *playerLvl, int *enemyLvl, int *exp, int *moedas, int *contraataque, int *burnMago, int *enemyIndex, int *nMobs, const char *mobs[], bool *bossFinal) {
    ...
    if (HPAtual <= 0) {
        ...
        *contraataque = 0;
        *burnMago = 0;
        reinicializarJogo(enemyIndex, nMobs, mobs, bossFinal, playerLvl, enemyLvl, contraataque, burnMago, levelUp);
        sleep(1);
        ...
        // Perguntar ao jogador se ele quer continuar jogando
        char resposta;
        printf("Você derrotou o boss final! Deseja continuar jogando? (s/n): ");
        scanf(" %c", &resposta);

        if (resposta != 's' && resposta != 'S') {
            *jogando = false;  // O jogador escolheu sair do jogo
        } else {
            *jogando = true;
            reinicializarJogo(enemyIndex, nMobs, mobs, bossFinal, playerLvl, enemyLvl, contraataque, burnMago, levelUp);
        }
