...
if (HPAtual <= 0) {
        ...
        *jogando = false;
        ...
        for (int i = *enemyIndex; i < *nMobs - 1; i++){
            mobs[i] = mobs[i + 1];
        }
        *nMobs--;
        if (*nMobs < 1){
...
void escalamento(int class, int *atkBase, int *res, int* forca, int* HPMaxima, int* HPAtual, int* manaMax, int* manaAtual, int* manaTempMax, int* manaTemp, int playerLvl, int enemyLvl, int* enemyHPMax, int* enemyHP, int* enemyBaseAtk, int* pocaoHP, int* pocaoMP){
...
int main(){
    ...
    //Array de strings
    char *mobs[] = {"TROLL","BRUXA","GOLEM","DRAGAO", NULL};
    ...
    do {
        ...
        do {
            ...
            if(enemyHP > 0){
                if (stun == false) {  
                    if (mobs[enemyIndex] == "TROLL"){
                    printf("%s USOU %s!\n", mobs[enemyIndex], atksTroll[rand() % (sizeof(atksTroll) / sizeof(atksTroll[0]))]);
                    sleep(1);
                    }
                    if(mobs[enemyIndex] == "BRUXA"){
                    printf("%s USOU %s!\n", mobs[enemyIndex], atksBruxa[rand() % (sizeof(atksBruxa) / sizeof(atksBruxa[0]))]);
                    sleep(1);
                    }
                    if(mobs[enemyIndex] == "GOLEM"){
                    printf("%s USOU %s!\n", mobs[enemyIndex], atksGolem[rand() % (sizeof(atksGolem) / sizeof(atksGolem[0]))]);
                    sleep(1);
                    }
                    if(mobs[enemyIndex] == "DRAGAO"){
                    printf("%s USOU %s!\n", mobs[enemyIndex], atksDragao[rand() % (sizeof(atksDragao) / sizeof(atksDragao[0]))]);
                    sleep(1);
                    }
            ...
            if (levelUp) {
                escalamento(class, &atkBase, &res, &forca, &HPMaxima, &HPAtual, &manaMax, &manaAtual, &manaTempMax, &manaTemp, playerLvl, enemyLvl, &enemyHPMax, &enemyHP, &enemyBaseAtk, &pocaoHP, &pocaoMP);
                levelUp = false;
            }
        } while (jogando);
